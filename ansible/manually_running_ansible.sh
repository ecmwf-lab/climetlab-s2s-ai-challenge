...
echo "   s2sadminpassword : password you want to give to the s2sadmin user"
echo "   s3accesskey" # TODO get the key from the .s3cfg file
echo "   s3secretkey" # TODO get the key from the .s3cfg file

ansible-playbook   -i florian-ansible, s2s.yml --extra-vars "ansible_become_pass=$(cat pass)" -e "s3accesskey=$(cat s3accesskey)" -e "s3secretkey=$(cat s3secretkey)" -e "s2sadminpassword=$(cat s2sadminpassword)" --private-key ~/.ssh/id_rsa_s2s
